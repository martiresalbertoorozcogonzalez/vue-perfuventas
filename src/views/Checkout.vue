	        	...
	        	<li v-for="item in this.$store.state.cart" class="media">
                  ...
                  <div class="media-body">
                    <h5 class="mt-0">{{item.productName}}
                       ...
                       <span class="float-right" @click="$store.commit('removeFromeCart',item)">X</span>
                    
                    </h5>
                    <p class="mt-0">{{item.productPrice | currency}}</p>
                    <p class="mt-0">{{item.productQuantity}}</p>
                  </div>
                </li>
	        </ul>
  ...
  export default {
    ...
    methods: {
       ...
       pay(){
         ...
         axios.get('https://us-central1-perfuventas-26ccb.cloudfunctions.net/CheckoutSession',{
              params: {
                products : data
              }
         })
